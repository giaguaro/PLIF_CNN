...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer3 = nn.Sequential(
            nn.Conv3d(628, 628, kernel_size=3, stride=1),
            nn.BatchNorm3d(628),
            nn.ReLU(inplace=True))
...
#        break
        ...
        if epoch != 0 and epoch % 10 == 0 and args.rank == 0: # only val and save on master node
...
def train(train_loader, model, criterion, optimizer, epoch, num_classes, classes_dict, args):
    ...
    for step, (inp,target) in enumerate(train_loader):
        ...
        target = [classes_dict.get(e, e) for e in target_temp]
